...
def concat_to_pbp_database(output_path, season, season_dict):
    
    outside_scope = True
    df_output = pd.read_csv(output_path)
    df_season = pd.read_csv('data/pbp_data/' + season_dict[season])
    game_day = df_output['Date'][0]

    for i in df_season.index:
        if df_season['Date'][i] > game_day:
            new_df = pd.concat([df_season.iloc[:i], df_output, df_season.iloc[i:]]).reset_index(drop=True)
            pd.DataFrame.to_csv(new_df, 'data/raw/sample_season.csv')
            outside_scope = False
            break
    
    if outside_scope:
        new_df = pd.concat([df_season, df_output]).reset_index(drop=True)
        pd.DataFrame.to_csv(new_df, 'data/raw/sample_season.csv')

PATH_GAMELINKS = 'data/raw/gamelinks.txt'
PATH_DONELINKS = 'data/raw/donelinks.txt'
PATH_PBP = "data/pbp_data/"

t = open(PATH_GAMELINKS,'r')
try:
    donet = open(PATH_DONELINKS,'r')
except:
    donet = open(PATH_DONELINKS,'w')
    donet.close()
    donet = open(PATH_DONELINKS,'r')
...
y = open('data/raw/output_pbp.csv','a')
if os.stat('data/raw/output_pbp.csv').st_size == 0:
   ...
   if t % 15 ==0 and t != 0:
      ...
      try:
        sleep(20)
         ...
         b = open(PATH_GAMELINKS,'w')
         for link in list(set(game_links) - set(used_links)):
             b.write(link + '\n')
         b.close()
         c = open(PATH_DONELINKS,'a')
